...
// # Old shenanigans, code changed a lot since
//
// ! Can't implement a From trait as the closure used can't take the shape of a specific generic
// but rather only use `impl Trait`, so the From<...> implementation requires us to use a generic
// which the closure by nature doesn't allow to, if you wanna spend some more hours doing this
// yourself sure.
//
// Hours wasted counter: 2 (probably)
//
// OMFG GOT IT WORKING WITH NIGHTLY SHENANIGANS, this was more of a compiler battle to see how far
// this could go, I don't actually encourage it.

// /// [`From`] trait to convert a [GenCacheStore] into a [TryCacheStore] that always succeeds. This
// /// might be useful for cache store abstractions that take a fallible store.
// ///
// /// This **HEAVILY** relies on nightly shenanigans, so its only available with the "shenanigans"
// /// feature, all because closures can't take a generic type and only work with `impl Fn*...`, which
// /// implementations don't allow. So there's no easy way to automatically wrap the generator's
// /// values into [`Ok`] [`Result`] variants.
// #[cfg(feature = "shenanigans")]
// impl<K, V, A, S: CacheStore<K, V>, F: Fn(&K, A) -> V> From<GenCacheStoreWrapper<K, V, A, S, F>>
//     for TryGenCacheStoreWrapper<K, V, (), A, CacheStoreAdapter<K, V, S>, FnAsOkFn<K, A, V, F>>
// {
//     fn from(value: GenCacheStoreWrapper<K, V, A, S, F>) -> Self {
//         let adapt: CacheStoreAdapter<K, V, S> = value.store.into();
//         let try_gen = FnAsOkFn {
//             func: value.generator,
//             phantom: PhantomData,
//         };

//         TryGenCacheStoreWrapper::from_store_and_generator(adapt, try_gen)
//     }
// }

// #[cfg(feature = "shenanigans")]
// impl<K, V, A, S: CacheStore<K, V>, F: Fn(&K, A) -> V, Fr: GenCacheStore<K, V, A>> From<Fr>
//     for TryGenCacheStoreWrapper<K, V, (), A, CacheStoreAdapter<K, V, S>, FnAsOkFn<K, A, V, F>>
// {
...
// fn from<K, V, S: CacheStore<K, V>, F, A, TryFn>(
//     value: GenCacheStore<K, V, S, F, A>,
// ) -> TryGenCacheStore<K, V, (), CacheStoreAdapter<K, V, S>, TryFn, A>
// where
//     F: Fn(&K, A) -> V,
//     for<'a> TryFn: Fn(&'a K, A) -> Result<V, ()>,
// {
//     let adapt: CacheStoreAdapter<K, V, S> = value.store.into();
//     let try_gen: TryFn = move |k: &K, a: A| Ok((value.generator)(k, a));

//     TryGenCacheStore::from_store_and_generator(adapt, try_gen)
// }

// struct FnAsOkFn<A: std::marker::Tuple, R, F: Fn<A, Output = R>> {
//     func: F,
//     phantom: PhantomData<A>,
// }

// impl<A: std::marker::Tuple, R, F: Fn<A, Output = R>> FnOnce<A> for FnAsOkFn<A, R, F> {
...
// impl<A: std::marker::Tuple, R, F: Fn<A, Output = R>> FnMut<A> for FnAsOkFn<A, R, F> {
//     extern "rust-call" fn call_mut(&mut self, args: A) -> Self::Output {
//         Ok(self.func.call(args))
//     }
// }

// impl<A: std::marker::Tuple, R, F: Fn<A, Output = R>> Fn<A> for FnAsOkFn<A, R, F> {
...
// #[cfg(feature = "shenanigans")]
// struct FnAsOkFn<K, V, R, F: for<'a> Fn<(&'a K, V), Output = R>> {
//     func: F,
//     phantom: PhantomData<(K, V)>,
// }

// #[cfg(feature = "shenanigans")]
// impl<K, V, R, F: for<'a> Fn<(&'a K, V), Output = R>> FnOnce<(&K, V)> for FnAsOkFn<K, V, R, F> {
//     type Output = Result<R, ()>;
...
// #[cfg(feature = "shenanigans")]
// impl<K, V, R, F: for<'a> Fn<(&'a K, V), Output = R>> FnMut<(&K, V)> for FnAsOkFn<K, V, R, F> {
...
// #[cfg(feature = "shenanigans")]
// impl<K, V, R, F: for<'a> Fn<(&'a K, V), Output = R>> Fn<(&K, V)> for FnAsOkFn<K, V, R, F> {
//     extern "rust-call" fn call(&self, args: (&K, V)) -> Self::Output {
//         Ok(self.func.call(args))
//     }
// }
